...
MainProgram::CmdResult MainProgram::cmd_add_road(std::ostream& output, MainProgram::MatchIter begin, MainProgram::MatchIter end)
{
    ...
    if (ok)
    {
        auto town1name = ds_.get_name(town1id);
        auto town2name = ds_.get_name(town2id);
...
void MainProgram::test_remove_road()
{
    if (random_towns_added_ > 0) // Don't do anything if there's no towns
    {
        auto id1 = n_to_id(random<decltype(random_towns_added_)>(0, random_towns_added_));
        auto id2 = n_to_id(random<decltype(random_towns_added_)>(0, random_towns_added_));
...
void MainProgram::add_random_towns(unsigned int size, Coord min, Coord max)
{
    for (unsigned int i = 0; i < size; ++i)
    {
        ...
        int x = random<int>(min.first, max.first);
        int y = random<int>(min.second, max.second);
...
MainProgram::CmdResult MainProgram::cmd_random_add(ostream& output, MatchIter begin, MatchIter end)
{
    ...
    {
        min.first = convert_string_to<unsigned int>(minxstr);
        min.second = convert_string_to<unsigned int>(minystr);
        max.first = convert_string_to<unsigned int>(maxxstr);
        max.second = convert_string_to<unsigned int>(maxystr);
    }
    else
    {
        ...
        if (!towns.empty())
        {
            ...
            {
                int x, y;
                std::tie(x,y) = ds_.get_coordinates(town);
...
MainProgram::CmdResult MainProgram::cmd_all_roads(std::ostream& output, MainProgram::MatchIter begin, MainProgram::MatchIter end)
{
    ...
    {
        auto coord1 = ds_.get_coordinates(p.first);
        auto coord2 = ds_.get_coordinates(p.second);
        auto dist = abs(coord1.first-coord2.first) + abs(coord1.second-coord2.second);
...
MainProgram::CmdResult MainProgram::cmd_roads(std::ostream& output, MainProgram::MatchIter begin, MainProgram::MatchIter end)
{
    string id = *begin++;
    assert( begin == end && "Impossible number of parameters!");
...
void MainProgram::print_town(TownID id, ostream& output)
{
    if (id != NO_ID)
    {
        auto name = ds_.get_name(id);
...
MainProgram::CmdResult MainProgram::cmd_road_cycle_route(std::ostream& output, MainProgram::MatchIter begin, MainProgram::MatchIter end)
{
    ...
    if (result.empty())
    {
        output << "No road cycles found." << std::endl;
    }
...
vector<MainProgram::CmdInfo> MainProgram::cmds_ =
{
    {"add_town", "ID Name (x,y) tax",
     "([a-zA-Z0-9]+)[[:space:]]+([a-zA-Z]+)[[:space:]]+\\(([0-9]+)[[:space:]]*,[[:space:]]*([0-9]+)\\)[[:space:]]+([0-9]+)", &MainProgram::cmd_add_town, nullptr },
    ...
    {"random_roads", "max_number_of_roads_to_add", "([0-9]+)",
    ...
    {"remove", "ID", "([A-Za-z0-9]+)", &MainProgram::cmd_remove, &MainProgram::test_remove },
    ...
    {"add_vassalship", "VassalID TaxerID",
     "([a-zA-Z0-9]+)[[:space:]]+([a-zA-Z0-9]+)", &MainProgram::cmd_add_vassalship, nullptr },
    {"vassals", "TownID", "([a-zA-Z0-9]+)", &MainProgram::cmd_vassals, &MainProgram::test_vassals },
    {"add_road", "Town1ID Town2ID",
     "([a-zA-Z0-9]+)[[:space:]]+([a-zA-Z0-9]+)", &MainProgram::cmd_add_road, nullptr },
    {"remove_road", "Town1ID Town2ID",
     "([a-zA-Z0-9]+)[[:space:]]+([a-zA-Z0-9]+)", &MainProgram::cmd_remove_road, &MainProgram::test_remove_road },
    {"roads", "TownID", "([a-zA-Z0-9]+)", &MainProgram::cmd_roads, &MainProgram::test_roads },
    {"clear_roads", "", "", &MainProgram::cmd_clear_roads, nullptr },
    {"taxer_path", "ID", "([A-Za-z0-9]+)", &MainProgram::cmd_taxer_path, &MainProgram::test_taxer_path },
    {"longest_vassal_path", "ID", "([A-Za-z0-9]+)", &MainProgram::cmd_longest_vassal_path, &MainProgram::test_longest_vassal_path },
    {"total_net_tax", "ID", "([A-Za-z0-9]+)", &MainProgram::cmd_total_net_tax, &MainProgram::test_total_net_tax },
    {"any_route", "Town1ID Town2ID",
     "([a-zA-Z0-9]+)[[:space:]]+([a-zA-Z0-9]+)", &MainProgram::cmd_any_route, &MainProgram::test_any_route },
    {"shortest_route", "Town1ID Town2ID",
     "([a-zA-Z0-9]+)[[:space:]]+([a-zA-Z0-9]+)", &MainProgram::cmd_shortest_route, &MainProgram::test_shortest_route },
    {"least_towns_route", "Town1ID Town2ID",
     "([a-zA-Z0-9]+)[[:space:]]+([a-zA-Z0-9]+)", &MainProgram::cmd_least_towns_route, &MainProgram::test_least_towns_route },
    {"road_cycle_route", "TownID",
     "([a-zA-Z0-9]+)", &MainProgram::cmd_road_cycle_route, &MainProgram::test_road_cycle_route },
...
MainProgram::CmdResult MainProgram::cmd_perftest(ostream& output, MatchIter begin, MatchIter end)
{
    ...
    vector<string> optional_cmds({"remove", "shortest_route", "least_towns_route", "road_cycle_route", "trim_road_network"});
    vector<string> nondefault_cmds({"all_towns", "all_roads", "remove", "find", "alphalist", "distlist",
                                   "mindist", "maxdist", "change_name", "nth_distance", "taxer_path", "towns_from", "longest_vassal_path",
                                    "total_net_tax", "vassals"});
        ...
        if (matched)
        {
            if (pos->func)
            {
               ...
               test_status_ = TestStatus::NOT_RUN;
                ...
                {
                    if (result.second.size() == 1 && result.second.front() == NO_ID)
                    {
                        ...
                        switch (result.first)
                        {
                            ...
                            case ResultType::PATH:
                            {
                                ...
                                for (TownID id : result.second)
                                {
                                    ...
                                    ++num;
                                    Coord coord = ds_.get_coordinates(id);
                                    if (prevcoord != NO_COORD)
                                    {
                                        dist += abs(coord.first-prevcoord.first) + abs(coord.second-prevcoord.second);
                                    }
                                    prevcoord = coord;
                                    output << dist << ": ";
                                    print_town(id, output);
                            ...
                            case ResultType::CYCLE:
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (((cycbeg+1) < (res.end()-2)) && (*(cycbeg+1) > *(res.end()-2)))
                                        {
...
void MainProgram::create_road_network()
{
    ...
    set<tuple<Dist, townssize, townssize>> roads;
    using roadssize = decltype(roads)::size_type;
    for (auto i1 = towns.size(); i1 != 0; --i1)
    {
        for (auto i2 = i1-1; i2 != 0; --i2)
        {
            Coord p1 = ds_.get_coordinates(towns[i1-1]);
            Coord p2 = ds_.get_coordinates(towns[i2-1]);
            Dist dist = abs(p1.first-p2.first)+abs(p1.second-p2.second);
...
//        auto roadi = roads.begin();
        ...
        if (s1 == s2) { continue; }
        Coord p1 = ds_.get_coordinates(towns[i1]);
        Coord p2 = ds_.get_coordinates(towns[i2]);
...
void MainProgram::add_random_roads(unsigned int random_roads)
{
    vector<pair<Coord, Coord>> addedroads;
    auto towns = ds_.all_towns();
    sort(towns.begin(), towns.end()); // Sort town IDs to get deterministic results
